...
bool LoadScene(FbxManager* pManager, FbxDocument* pScene, const char* pFilename, bool verbose)
{
    ...
    if (verbose) FBXSDK_printf("FBX file format version for this FBX SDK is %d.%d.%d\n", lSDKMajor, lSDKMinor, lSDKRevision);

    if (lImporter->IsFBX() && verbose)
    {
        FBXSDK_printf("FBX file format version for file '%s' is %d.%d.%d\n\n", pFilename, lFileMajor, lFileMinor, lFileRevision);
